...
class OAuth2ClientCredentialsAuth(httpx.Auth):
    ...
    def _get_new_token(self):
        ...
        if not client_id or not client_secret:
            self.logger.error("Client ID or client secret not found in environment variables.")
            return
    ...
    def auth_flow(self, request: httpx.Request) -> Generator[httpx.Request, httpx.Response, None]:
        if not self._access_token or time.time() >= self._token_expiry_time:
            self.logger.info("OAuth2 token expired or not present, requesting new token...")
            self._get_new_token()

        if self._access_token:
            request.headers["Authorization"] = f"Bearer {self._access_token}"
        else:
            self.logger.warning("No OAuth2 access token available, proceeding without authentication.")
        yield request
...
def create_auth_handler(auth_config: Union[BearerAuthConfig, OAuth2ClientCredentialsConfig], logger: logging.Logger) -> httpx.Auth | None:
    if isinstance(auth_config, BearerAuthConfig):
        api_key = os.getenv(auth_config.api_key_env_var)
        if not api_key:
            logger.error(f"API key not found for environment variable: {auth_config.api_key_env_var}")
            return None
