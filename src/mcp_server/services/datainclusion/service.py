...
def _get_datainclusion_client() -> httpx.AsyncClient:
    ...
    if not api_key:
        raise ValueError("La variable d'environnement DATAINCLUSION_API_KEY est requise.")
...
def api_call_handler(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Erreur HTTP dans '{tool_name}': {e.response.status_code} - {e.response.text}")
            raise ModelRetry(f"Erreur de communication avec l'API Data Inclusion ({e.response.status_code}): {e.response.text}") from e
        except httpx.HTTPError as e:
            logger.error(f"Erreur de communication dans '{tool_name}': {e}")
            raise ModelRetry(f"Erreur de communication avec l'API Data Inclusion: {e}") from e
...
async def fetch_reference_values(
    category: Literal["themes", "costs", "reception_modes", "mobilization_modes", "mobilizing_persons", "target_audience", "networks", "service_types"]
) -> List[ReferenceItem]:
...
async def list_all_structures(
    ...
    """Liste les structures d'inclusion, avec la possibilité de filtrer par réseau."""
    params = {"size": limit}
    if network:
...
async def list_all_services(
    ...
    """Liste les services d'inclusion, avec des filtres non géographiques comme les thématiques, les frais ou le public cible."""
    params = {"size": limit}
    if themes:
...
async def search_services(
    ...
    Le lieu (`location_text`) et la thématique (`themes`) sont obligatoires. La recherche est limitée à 10 résultats. Il faut passer qu'une seule chaîne de référentiel pour les thématiques.
    """
    if isinstance(themes, str): themes = [themes]
    if isinstance(target_audience, str): target_audience = [target_audience]

    # MODIFICATION: La limite est maintenant fixée à 10 et n'est plus un paramètre de la fonction.
    params = {"size": 10}
    
    # Le paramètre location_text est maintenant obligatoire
    async with httpx.AsyncClient() as geo_client:
        geo_response = await geo_client.get("https://api-adresse.data.gouv.fr/search/", params={"q": location_text, "limit": 1})
        geo_response.raise_for_status()
        geo_data = geo_response.json()
    
    if geo_data.get("features") and (insee_code := geo_data["features"][0]["properties"].get("citycode")):
        params["code_commune"] = insee_code
    else:
        raise ValueError(f"Le géocodage pour '{location_text}' n'a pas retourné de code INSEE valide.")

    # Le paramètre themes est maintenant obligatoire
    params["thematiques"] = ",".join(themes)
    if target_audience: params["publics"] = ",".join(target_audience)
    ...
    for item in items:
        ...
        address_parts = [structure.get('adresse'), structure.get('code_postal'), structure.get('commune')]
...
def create_datainclusion_mcp_server() -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions="Ce serveur fournit des outils pour rechercher et consulter des données sur les structures et services d'inclusion en France. Utilisez `search_services` pour des recherches géolocalisées et `list_*` pour des listes générales. La recherche de services (`search_services`) requiert obligatoirement un lieu et une thématique."
    )
