...
class MCPFactory:
    ...
    async def _load_openapi_spec(self) -> None:
        ...
        """
        self.logger.info(f"Loading OpenAPI specification from URL: '{self.settings.OPENAPI_URL}'...")
        ...
        try:
            ...
            self.logger.info(f"Successfully parsed {len(self.http_routes)} HTTP routes from OpenAPI specification")
            ...
            self.openapi_spec = limit_page_size_in_spec(self.openapi_spec, logger=self.logger, max_size=25)
            
        except httpx.RequestError as e:
            self.logger.error(f"Failed to fetch OpenAPI specification from '{self.settings.OPENAPI_URL}'.")
            self.logger.error(f"Details: {e}")
            raise
            
        except json.JSONDecodeError as e:
            self.logger.error(f"Invalid JSON in the response from '{self.settings.OPENAPI_URL}'.")
            self.logger.error(f"Details: {e}")
            raise
    ...
    async def _create_api_client(self) -> None:
        ...
        self.api_client = create_api_client(base_url, self.logger, self.settings.DATA_INCLUSION_API_KEY)
    ...
    def _configure_auth(self) -> None:
        ...
        if secret_key and secret_key.strip():
            self.logger.info("Secret key found - configuring Bearer Token authentication...")
            try:
                ...
                if secret_key.strip().startswith("-----BEGIN") and "PRIVATE KEY" in secret_key:
                    ...
                    self.logger.info(f"🔑 Test Bearer Token (for development): {test_token}")
                
                self.logger.info("✓ Bearer Token authentication configured successfully")
                self.logger.info("   - Audience: datainclusion-mcp-client")
                self.logger.info("   - Server will require valid Bearer tokens for access")
            ...
            self.logger.warning("MCP_SERVER_SECRET_KEY not set - server will run WITHOUT authentication")
            self.logger.warning("⚠️  All clients will have unrestricted access to the server")
    ...
    def _create_mcp_server(self) -> FastMCP:
        ...
        self.logger.info(f"Creating FastMCP server '{self.settings.MCP_SERVER_NAME}'...")
        ...
        mcp_server = FastMCP.from_openapi(
            ...
            mcp_component_fn=lambda route, component: discover_and_customize(route, component, self.logger, self.op_id_to_mangled_name)
        )
    ...
    async def _enrich_tools(self, mcp_server: FastMCP) -> None:
        ...
        if not hasattr(self, 'custom_tool_names'):
            raise ValueError("Custom tool names must be configured before enriching tools")
