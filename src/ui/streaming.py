...
async def process_agent_with_history_and_tools(
    ...
    try:
        ...
        async with cl.Step(name="Traitement avec Agent MCP", type="llm") as step:
            ...
            result_text = str(result.output) if hasattr(result, 'output') else str(result.data)
            ...
            lines = result_text.splitlines(True)  # Garde les caractères de nouvelle ligne
            for line in lines:
                await asyncio.sleep(0.05)  # Délai entre les lignes pour l'effet de streaming
...
async def process_agent_stream_with_tools_display(
    ...
    try:
        ...
        async with agent.run_stream(message, message_history=message_history or []) as result:
                    ...
                    for part in msg.parts:
                        ...
                        # Détection des résultats d'outils  
                            ...
                            for tool_info in tool_calls_info:
                                if tool_info.get('tool_call_id') == getattr(part, 'tool_call_id', None):
            ...
            for i, tool_info in enumerate(tool_calls_info, 1):
                ...
                if tool_result:
                    async with cl.Step(
                        ...
                        result_step.output = str(tool_result)[:1000]  # Limiter à 1000 chars pour l'affichage
...
async def process_agent_modern_with_history(
    ...
    try:
        ...
        try:
            fallback_messages = await process_agent_with_history_and_tools(agent, message, message_history)
