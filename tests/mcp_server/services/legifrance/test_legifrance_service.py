...
class TestRechercherTextesJuridiques:
    ...
    @patch("pylegifrance.fonds.loda.Loda.search")
    @patch("pylegifrance.fonds.juri.JuriAPI.search")
    async def test_rechercher_textes_juridiques_success(
        self, mock_juri_search, mock_loda_search
    ):
        ...
        mock_juri_result.title = (
            "Décision Test"  # Changement ici: title au lieu de titre
        )

        mock_loda_search.return_value = [mock_loda_result]
        mock_juri_search.return_value = [mock_juri_result]
    ...
    async def test_rechercher_textes_juridiques_with_error(
        ...
        mock_loda_search.side_effect = ValueError("Erreur de recherche")
    ...
    @patch("pylegifrance.fonds.loda.Loda.search")
    @patch("pylegifrance.fonds.juri.JuriAPI.search")
    async def test_rechercher_textes_juridiques_with_juri_error(
        self, mock_juri_search, mock_loda_search
    ):
        ...
        mock_juri_search.side_effect = ValueError("Erreur de recherche JURI")
...
class TestConsulterTexteLoiDecret:
    ...
    async def test_consulter_texte_loi_decret_with_id_enrichment(
        ...
        # Vérifications
        mock_loda_service.fetch.assert_called_with("LEGITEXT000000000001")
        assert result is not None
...
class TestConsulterConventionCollective:
    ...
    async def test_consulter_convention_collective_with_id_enrichment(
        ...
        # Vérifications
        mock_loda_service.fetch.assert_called_with("KALITEXT000000000001")
        assert result is not None
